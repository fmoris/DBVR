...
## [v1.5.2] — 2026-04-01 · Skill metaquest-webxr activado

### Agregado
- **Skill `metaquest-webxr`**: Activado para el proyecto. Proporciona guía experta en desarrollo WebXR para Meta Quest.
- Referencias disponibles en `.clawhub/metaquest-webxr/`: hand-tracking, webxr-features, performance, deployment, scene-architecture, project-setup.
- Actualizado `CLAUDE.md` con sección de skills activos.
...
## [v1.5.1] — 2026-04-01 · Manos 3D visibles en VR + Sin puntero láser

### Modificado
- **Manos 3D en VR**: cambiado `disableDefaultHandMesh: false` e `invisible: false` en la configuración de hand tracking para mostrar las mallas 3D de las manos en lugar de los controladores.
- **Eliminado puntero por defecto**: deshabilitado `WebXRFeatureName.POINTER_SELECTION` para quitar los láseres de los controladores (no se necesitan con hand tracking).
- **Deshabilitada teleportación**: deshabilitado `WebXRFeatureName.TELEPORTATION` ya que el juego es estático en primera persona.

### Archivos modificados
`Game.ts` · `WebXRManager.ts`
