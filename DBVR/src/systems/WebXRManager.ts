...
export class WebXRManager {
  ...
  async initializeWebXR(): Promise<boolean> {
    try {
      ...
      this.xrHelper = await this.scene.createDefaultXRExperienceAsync({
        ...
        optionalFeatures: ['hand-tracking', 'dom-overlay', 'dom-overlay-for-handheld'],
      });
  ...
  private extractHandJoints(hand: any): HandJoints {
    // Mapeo de índices XR a articulaciones
   ...
   */
  private getJointPosition(hand: any, jointIndex: number): { x: number; y: number; z: number } {
    try {
